...
class CiprMainInitClient {
  ...
  async initFinalize() {
    return this.ciprMainClient.initFinalize(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptCiprMainInitDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptCiprMainInitDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuCipr init started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new CiprMainInitClient(dtoIn.lockSecret);
  ...
  uuCipr = await mainContext.initFinalize();

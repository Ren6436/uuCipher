...
const Names = {
  ...
  SCRIPT_NAME: "CiprMainInitRollback",
};
/*@@viewOff:names*/
...
/*@@viewOn:scriptClient*/
class CiprMainInitRollbackClient {
  ...
  async initFinalizeRollback() {
    return this.ciprMainClient.initFinalizeRollback(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptCiprMainInitRollbackDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptCiprMainInitRollbackDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuCipr initRollback started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new CiprMainInitRollbackClient(dtoIn.lockSecret);
  ...
  uuCipr = await mainContext.initFinalizeRollback();

...
const BeingInitializedAuthorized = createVisualComponent({
  ...
  render() {
    ...
    const { data: uuCipr } = useSubAppData();
    ...
    if (uuCipr?.temporaryData?.dtoIn.mode) {
      uuCipr.temporaryData.dtoIn.mode = "retry";
    }

    const [showConsole, setShowConsole] = useState(!!uuCipr.consoleCode);
    const [retryInputData, setRetryInputData] = useState(
      JSON.stringify({ force: false, mode: "retry", data: uuCipr?.temporaryData?.dtoIn }, null, 2),
    );
    const [rollbackInputData, setRollbackInputData] = useState(
      JSON.stringify({ force: true, mode: "rollback", data: {} }, null, 2),
    );
    const consoleBaseUri = uuCipr.temporaryData.progressMap.uuConsoleUri;
    const progressCode = uuCipr.temporaryData.progressMap.progressCode;
    const consoleCode = uuCipr.temporaryData.progressMap.consoleCode;
